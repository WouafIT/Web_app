module.exports = (function() {
	...
	self.formatDate = function(date, format) {
		...
		if (format == 'short') {
			...
			if (locale == 'fr') {
				return date.getDate() +' '+ i18n.t(months[date.getMonth()]) +' '+ date.getFullYear();
			} else {
				return i18n.t(months[date.getMonth()]) +' '+ date.getDate() +', '+ date.getFullYear();
			}
		}
	};
	self.formatTime = function(date, format) {
		format = format || 'short';
